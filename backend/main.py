...
forensic_payload = f"Identity:\nName: {comp_name}\nCRN: {crn}\nAddress: {clean_address}\n\nOfficers:\n" + "\n".join(officer_lines) + "\n\nTimeline:\n" + "\n".join(filing_lines)
        ...
        if anthropic_client:  
            msg = anthropic_client.messages.create(
                ...
                max_tokens=3500,
                temperature=0.1,
                system="""You are a premier senior forensic corporate investigator for an elite intelligence council.
Organize your disclosure into two distinct, high-impact sections:

- ## I. KEY CORPORATE METRICS & EXECUTIVE ROSTER
- Provide a clear, prosaic, line-by-line list outlining the core facts of the entity (Name, CRN, Status, and Registered Address) followed explicitly by a complete list of all named directors and officers alongside their active corporate roles. Do not summarize or synthesize here—keep it purely factual, clean, and direct. Every single line in this section must start with a hyphen list marker.

- ## II. STRATEGIC TRAJECTORY NARRATIVE
- Below the list, deliver a comprehensive, highly fluid, human narrative intelligence brief. Connect chronological events, explain the timeline trajectory of leadership shifts, cross-reference key filing events, and analyze corporate momentum. Avoid generic references; explicitly identify individuals by their real names whenever analyzing appointments or resignations. Write in sophisticated, rich, human analytical paragraphs. Every single line of this prose section must also start with a hyphen list marker.""",
